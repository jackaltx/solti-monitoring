...
- [x] Task: Research InfluxDB V3 architecture, features, and breaking changes from V2.
  ...
  - [x] Sub-task: Understand new authentication mechanisms (if any).
  - [x] Sub-task: Investigate data model changes and query language differences.
- [x] Task: Analyze the existing `influxdb` Ansible role.
  - [x] Sub-task: Map current role variables and tasks to InfluxDB V2 features.
  - [x] Sub-task: Identify areas requiring modification for V3 compatibility.
- [x] Task: Design the migration strategy for the Ansible role.
  - [x] Sub-task: Determine if a new `influxdb3` role is required or if the existing `influxdb` role can be updated.
  - [x] Sub-task: Outline changes to default variables, tasks, and templates.
  - [x] Sub-task: Plan for Molecule test updates for InfluxDB V3.
